...
| `-no-confirm` | `VASTAI_NO_CONFIRM` | | Comma-separated tools exempted from `-confirm`, for unattended agents: `vast_create_instance`, `vast_destroy_instance`, `vast_execute`, `vast_create_ssh_key`, `vast_attach_ssh_key`, or `all`. The `vast_` prefix is optional. An unknown name, or one that never prompts, refuses to start. Exempted tools act on the first call with no preview, so keep `-max-dph` / `-max-instances` set when exempting creates.
