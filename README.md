...
| --- | --- | --- | --- |
| `-confirm` | `VASTAI_CONFIRM` | `true` | `vast_create_instance`, `vast_destroy_instance`, and `vast_execute rm …` return a cost/impact preview and ask the user through the client's confirmation prompt (MCP elicitation). A decline is final. Clients without elicitation may pass `confirm: true` on stdio or loopback only. |
| `-max-dph` | `VASTAI_MAX_DPH` | `0` (off) | Reject creates above this $/hr. Checked against the live offer before the request and against the real instance price after. |
